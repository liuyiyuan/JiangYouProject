...
/** 是否已提交了询前表单 */
//@property (nonatomic, assign) BOOL             iscommitAskTabel;
...
-(void)createChatView{
    ...
    topline.backgroundColor =[ZCUITools zcgetDynamicColor];
...
-(void) checkInitAction{
    ...
    // 判断是否需要重新初始化  [ZCSobotCore checkInitParameterChanged]
...
//        }
        if([self getPlatformInfo].messageArr!=nil && [self getPlatformInfo].messageArr.count > 0){
            [_listArray addObjectsFromArray:[self getPlatformInfo].messageArr];
        }
...
//            _receivedName = [ZCIMChat getZCIMChat].libConfig.senderName;
            [self setTitleName:[self getZCLibConfig].senderName];
            _receivedName = [self getZCLibConfig].senderName;
...
//        }else{
//            [ZCIMChat getZCIMChat].libConfig.type = 0;
//        }
//
//        [_listArray removeAllObjects];
//        _listArray = nil;
    ...
    if(isStartConnectSockt){
        ...
        [self getPlatformInfo].messageArr = _listArray;
    }
    _cidsArr = nil;
    _listArray = nil;
...
-(void)onReceivedMessage:(ZCLibMessage *)message unReaded:(int)num object:(id)obj showType:(ZCReceivedMessageType)type{
    if(![[self getPlatformInfo].appkey isEqual:zcLibConvertToString(obj[@"appId"])]){
        return;
    }
...
//        [_zcKeyboardView  setRobotViewStatusType:AGAINACCESSASTATUS];
        ...
        //  obj !=nil && [obj isKindOfClass:[NSNumber class]] && ![obj boolValue]
        ...
        // [ZCIMChat getZCIMChat].messageArr
        for(ZCLibMessage *item in [self getPlatformInfo].messageArr){
            if(item.tipStyle>0){
...
//            [ZCIMChat getZCIMChat].libConfig.isArtificial = NO;
            [self getZCLibConfig].isArtificial = NO;
        }
    ...
    [[self getZCAPIServer] getChatUserCids:[ZCLibClient getZCLibClient].libInitInfo.scopeTime config:[self getZCLibConfig] start:^{
        ...
        if(zcLibIs_null(arr)  || arr.count == 0){
            ...
            for (NSString *itemCid in arr) {
                if(_currentCid == nil && [[self getZCLibConfig].cid isEqual:itemCid] && _cidsArr.count>0){
                    _currentCid = [_cidsArr lastObject];
                    break;
                }
...
-(void)getHotGuideWord{
    ...
    __weak ZCUIChatController *safeVC = self;
    [[self getZCAPIServer] getHotGuide:[self getZCLibConfig] Parms:param start:^(ZCLibMessage *message) {
...
//       [self showCustomActionSheet:ServerSatisfcationNolType andDoBack:NO isInvitation:0 Rating:5 IsResolved:0];
    ...
    if (type == ZCChatCellClickTypeLeaveMessage) {
        // 不直接退出SDK
        [self goLeaveMessageVC:2 isShowToat:NO tipMsg:@""];
    }
...
//        [self.zcKeyboardView getKeyBoardViewStatus] == AGAINACCESSASTATUS
        ...
        [[self getZCAPIServer] sendMessage:model.richModel.msg questionId:@"" msgType:model.richModel.msgType duration:model.richModel.duration config:[self getZCLibConfig] robotFlag:[ZCLibClient getZCLibClient].libInitInfo.robotId start:^(ZCLibMessage *message) {
            ...
            if(![self getZCLibConfig].isArtificial && sendCode==ZC_SENDMessage_New){
                NSInteger index = [_listArray indexOfObject:model];
...
//        * 正在发送的消息对象，方便更新状态
        ...
        if ([self getZCLibConfig].isArtificial || [dict[@"ishotguide"] intValue] == 1) {
            [self sendMessage:dict[@"title"] questionId:@"" type:ZCMessageTypeText duration:@""];
            return;
        }
        
        // 发送给机器人
        [[self getZCAPIServer] sendToRobot:dict[@"requestText"] showText:dict[@"title"] questionStr:dict[@"question"] questionFlag:2 msgType:model.richModel.msgType questionId:@"" config:[self getZCLibConfig] robotFlag:[ZCLibClient getZCLibClient].libInitInfo.robotId duration:@"" start:^(ZCLibMessage *message) {
            sendMessage  = message;
 ...
 */
-(void)goLeaveMessageVC:(NSInteger) isExist  isShowToat:(BOOL) isShow  tipMsg:(NSString *)msg{
...
-(void) sendMessage:(NSString *)text questionId:(NSString*)question type:(ZCMessageType) type duration:(NSString *) time{
    ...
    if(self.zckitInfo.isOpenActiveUser && type == ZCMessageTypeText && ![self getZCLibConfig].isArtificial && [self getZCLibConfig].type!=1){
        if(!zcLibIs_null(self.zckitInfo.activeKeywords) && [self.zckitInfo.activeKeywords objectForKey:text]){
         
            //        ZCLibMessage *sendModel = nil;
            // action 为0
            [self createMessageToArrayByAction:0 type:0 name:_receivedName face:[self getZCLibConfig].face tips:0 content:text];
             // 刷新页面，在去转人工
            [_zcKeyboardView turnUserServer:NO];
            return;
        }
...
//            [_zcKeyboardView setRobotViewStatusType:ONLYUSERNOLINESTATUS];
    ...
    if(type == ZCKeyboardOnClickCloseSkillSet){
        ...
        if([self getZCLibConfig].type == 2){
            [self  goBackIsKeep];
        }else if([self getZCLibConfig].type == 4){
            // 添加机器人欢迎语
            [self showRobotHello];
        }
    }
    ...
    if(type==ZCKeyboardOnClickTurnUser){
        ...
        if([self getPlatformInfo].waitintMessage!=nil &&  [[self getZCLibConfig].cid isEqual:[self getPlatformInfo].waitintMessage.cid]){
            isWaiting = YES;
        }
        
        
        // 2.4.2新增询前表单 在转人工的事件进行操作
        if (!isWaiting && ![(NSString *)obj isEqualToString:@"isinitiativeTurn"]) {
            // 关闭加载动画
            [[ZCUIToastTools shareToast] dismisProgress];
            [[self getZCAPIServer] getAskTabelWithUid:[self getZCLibConfig].uid start:^{
                ...
                @try{
                    ...
                    if ([zcLibConvertToString(dict[@"code"]) intValue] == 1 && [zcLibConvertToString(dict[@"data"][@"openFlag"]) intValue] == 1) {
                        ...
                        if ([(NSString *)obj isEqualToString:@"clearskillId"]) {
                            askVC.isclearskillId = YES;
                        }
                        __weak ZCUIChatController *weakSelf = self;
                        askVC.trunServerBlock = ^(BOOL isback) {
                            if (isback && [weakSelf getZCLibConfig].type == 2) {
                                // 返回当前页面
                                [weakSelf  goBackIsKeep];
                            }else{
                                if (isback) {
                                    return ;
                                }else{
                                    // 去执行转人工的操作
                                    [weakSelf trunUserWith:isWaiting];
                                }
                        ...
                        if (self.navigationController) {
                            ...
                            UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:askVC];
...
//        ZCLibMessage *message = [ZCIMChat getZCIMChat].waitMessage;
        ZCLibMessage *message = [self getPlatformInfo].waitintMessage;
        if(message!=nil){
            message.isRead = NO;
            [self addReceivedNameMessageToList:message];
        }
...
-(void)trunUserWith:(BOOL)isWaiting{
...
//            [_zcKeyboardView setRobotViewStatusType:ONLYUSERNOSATISFACTION];
        ...
        if ([self getZCLibConfig].msgFlag == 0) {
            //  跳转到留言不直接退出SDK
            [self goLeaveMessageVC:2 isShowToat:YES tipMsg:zcLibConvertToString(dict[@"msg"])];
        }
        return;
    }
