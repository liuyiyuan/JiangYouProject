...
-(CGFloat)InitDataToView:(ZCLibMessage *)model time:(NSString *)showTime{
    ...
    if(model.senderType==0){
        ...
        // 左端盖宽度
        NSInteger leftCapWidth = bgImage.size.width * 0.4f;
        // 顶端盖高度
...
//        bgImage = [ZCUITools zcuiGetBundleImage:@"zcicon_pop_green_normal"];
//        bgImage=[bgImage resizableImageWithCapInsets:UIEdgeInsetsMake(0, 5, 0, 1)];
        // 右边气泡绿色
...
//                model.senderFace = [ZCIMChat getZCIMChat].libConfig.senderFace;
        ...
        NSInteger leftCapWidth = bgImage.size.width * 0.6f;
        // 顶端盖高度
        NSInteger topCapHeight = bgImage.size.height * 0.5f;
        // 重新赋值
        bgImage = [bgImage stretchableImageWithLeftCapWidth:leftCapWidth topCapHeight:topCapHeight];
