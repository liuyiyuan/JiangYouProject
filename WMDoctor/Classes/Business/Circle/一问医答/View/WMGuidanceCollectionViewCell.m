//
//  WMGuidanceCollectionViewCell.m
//  Micropulse
//
//  Created by 茭白 on 2016/11/1.
//  Copyright © 2016年 iChoice. All rights reserved.
//

#import "WMGuidanceCollectionViewCell.h"

@implementation WMGuidanceCollectionViewCell

- (void)awakeFromNib {
    [super awakeFromNib];
    //self.GuidanceImage.contentMode = UIViewContentModeScaleAspectFill;
    self.GuidanceImage.image=[UIImage imageNamed:@"im_shangchuantupian2"];
    self.GuidanceImage.layer.cornerRadius=4;
    self.GuidanceImage.layer.masksToBounds=YES;
    // Initialization code
    
    
    
    
    
}

-(void)setupView{
    
    
    
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        CAShapeLayer *borderLayer = [CAShapeLayer layer];
        borderLayer.frame = CGRectMake(0, 0, self.showView.frame.size.width, self.showView.frame.size.height);
        //    borderLayer.position = CGPointMake(self.showView.centerX, self.showView.centerY);
        
        borderLayer.path = [UIBezierPath bezierPathWithRect:self.showView.frame].CGPath;
        borderLayer.path = [UIBezierPath bezierPathWithRoundedRect:borderLayer.frame cornerRadius:4].CGPath;
        
        
        borderLayer.lineWidth = 2;
        //虚线边框
        borderLayer.lineDashPattern = @[@12, @6];
        //实线边框
        //    borderLayer.lineDashPattern = nil;
        borderLayer.fillColor = [UIColor clearColor].CGColor;
        borderLayer.strokeColor = [UIColor colorWithHexString:@"cccccc"].CGColor;
        [self.showView.layer addSublayer:borderLayer];

    });
    

    
}
@end
