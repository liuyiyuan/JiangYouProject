...
@interface WMTownSelectViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation WMTownSelectViewController
...
- (void)setupData{
    ...
    [apiManager loadDataWithParams:@{@"parentId":self.areaStrCode} withSuccess:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if (models.areas.count <= 0) {
            ...
            NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:self.areaStrCode,@"areaCodeStr",@"3",@"areaStyle", nil];
            [self saveArea];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    self.saveModel.streetId = ((WMAreaModel *)_allArr[indexPath.row]).areaId;
    self.saveModel.area = areaStrTemp;
    WMCountrySelectViewController * goVC = [[WMCountrySelectViewController alloc]init];
