...
@implementation ACOGFourCollectionViewCell
-(id)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [self.topMarkBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.bottom.equalTo(self.markBtn.mas_centerY).offset(-15);
            make.left.equalTo(self.contentView.mas_left).offset(5);
            make.size.equalTo([NSValue valueWithCGSize:CGSizeMake(15, 15)]);
        }];
        ...
        [self.btmMarkBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.markBtn.mas_centerY).offset(15);
            make.left.equalTo(self.contentView.mas_left).offset(5);
            make.size.equalTo([NSValue valueWithCGSize:CGSizeMake(15, 15)]);
        }];
        ...
        [self.contentView addSubview:self.topTipLable];
        [self.topTipLable mas_makeConstraints:^(MASConstraintMaker *make) {
            make.bottom.equalTo(self.topLable.mas_top).offset(-10);
            make.left.equalTo(self.contentView.mas_left).offset(5);
            make.right.equalTo(self.contentView.mas_right).offset(-5);
...
-(UILabel *)topTipLable {
    if (!_topTipLable) {
        _topTipLable = [[UILabel alloc] init];
        _topTipLable.font = [UIFont systemFontOfSize:9];
        _topTipLable.textColor = RGBA(64, 64, 64, 1);
        _topTipLable.numberOfLines = 0;
        [_topTipLable setLineBreakMode:NSLineBreakByCharWrapping];
