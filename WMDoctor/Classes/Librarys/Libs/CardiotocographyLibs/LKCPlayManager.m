...
@property (nonatomic, strong) id<LKCPlayManagerUIDelegate> delegate;//定义一个委托变量
...
+ (instancetype)sharedPlayManager
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        [[NSRunLoop currentRunLoop] addTimer:playManager.callDelegateTimer forMode:NSRunLoopCommonModes];//将该定时器加入到mainRunLoop线程中，用runloop后这个线程就不会退出了,会停止在这里？调试结果是没有，直接下去了，靠！
...
{
    if (_delegate && self.player && [self.player isPlaying]) {
