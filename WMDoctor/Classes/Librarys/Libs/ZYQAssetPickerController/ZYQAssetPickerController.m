...
+ (void)initialize
{
    checkedIcon     = [UIImage imageNamed:@"ic_duihao"];//[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"ZYQAssetPicker.Bundle/Images/%@", @"AssetsPickerChecked"]]];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(shouldTap)]) {
        if (![_delegate shouldTap]&&!_selected) {
           //在这里加一个代理，用来提示图片数目已经够了
            if (self.delegate &&[self.delegate respondsToSelector:@selector(isWarningOverPictureNumber)]) {
                [self.delegate isWarningOverPictureNumber];
            }
            return;
        }
...
-(void)isWarningOverPictureNumber{
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(isWarningOverPictureNumberAgain)]) {
        [_delegate isWarningOverPictureNumberAgain];
    }
}
...
-(void)isWarningOverPictureNumberAgain{
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(isWarningOverPictureNumberMore)]) {
        [_delegate isWarningOverPictureNumberMore];
    }
}
@end
...
- (void)setupButtons
{
    ...
    UIBarButtonItem *buttonItem=[[UIBarButtonItem alloc] initWithCustomView:[self finishPickingAssetsView]];
    
    buttonItem.width=50;
    self.navigationItem.rightBarButtonItems = @[buttonItem];
    
}
-(UIView *)finishPickingAssetsView{
    ...
    UITapGestureRecognizer * PrivateLetterTap=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(finishPickingAssets)];
...
-(void)isWarningOverPictureNumberMore{
     ZYQAssetPickerController *vc = (ZYQAssetPickerController *)self.navigationController;
    NSString *strNumber=[NSString stringWithFormat:@"您最多只能选择%ld张图片",(long)vc.maximumNumberOfSelection];
    [WMHUDUntil showMessageToWindow:strNumber];
...
-(UIView *)sureView{
    ...
    label.textAlignment=NSTextAlignmentCenter;
    [view addSubview:label];
    UITapGestureRecognizer * PrivateLetterTap=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(dismiss)];
    PrivateLetterTap.numberOfTouchesRequired = 1; //手指数
    PrivateLetterTap.numberOfTapsRequired = 1; //tap次数
    [view addGestureRecognizer:PrivateLetterTap];
    return view;
